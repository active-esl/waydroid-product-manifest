...
and must not be staged again. The `userdebug` pair from
[run 35084191413](https://github.com/active-esl/waydroid-product-manifest/actions/runs/35084191413)
is selected on Jaguar Screen. The board passed a warm-reboot display test with
the Active-Edge splash held until LineageOS appeared, using a bench hotpatch
on Foundries target 2943. Build the captured host fix into a successor target,
verify the Android artifact/release evidence, and repeat the same test after
OTA with bench overrides removed. Cold boot, acceleration, media, memory and
lifecycle remain separate gates. Build the Android `user` variant only after
the `userdebug` integration gates pass.
